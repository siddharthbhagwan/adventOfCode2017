...
loi = loi
  ...
  .split('\n')
  .forEach(instruction => {
    let splitI = instruction.split(' ');
    let testCondition = (register[splitI[4]] || 0) + splitI[5] + splitI[6];

    if (eval(testCondition)) {
      if (splitI[1] === 'inc') {
        register[splitI[0]] = (register[splitI[0]] || 0) + parseInt(splitI[2]);
      } else {
        register[splitI[0]] = (register[splitI[0]] || 0) - parseInt(splitI[2]);
      }
    }
    ...
    if (Math.max.apply(null, keysArray) > maxima) {
      maxima = Math.max.apply(null, keysArray);
    }
  });
